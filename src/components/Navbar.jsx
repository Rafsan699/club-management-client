    ...
    <header className={`${darkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-200/85'} backdrop-blur-2xl border-b fixed inset-x-0 z-50 shadow-sm transition-all duration-300 ${showNavbar ? 'top-0' : '-translate-y-full'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-2.5 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center relative">
          ...
          {showExploreMenu && (
            <div className={`absolute top-12 left-0 w-[95vw] sm:w-[900px] ${darkMode ? 'bg-slate-900/98 border-slate-800 text-slate-100' : 'bg-white/98 border-slate-200/80 text-slate-900'} backdrop-blur-2xl border rounded-3xl shadow-[0_25px_70px_rgba(0,0,0,0.15)] p-5 sm:p-8 z-50 max-h-[82vh] overflow-y-auto custom-scrollbar`}>
              ...
              <div className={`flex items-center justify-between pb-4 mb-5 border-b ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`font-black ${darkMode ? 'text-white' : 'text-slate-900'} text-lg sm:text-xl uppercase tracking-wider`}>
                      Club Directory & Navigation
                    </h3>
                    <p className={`text-xs font-bold ${darkMode ? 'text-slate-400' : 'text-slate-500'} mt-0.5`}>Browse all club resources, committees and portals instantly</p>
                  </div>
                </div>
                <button 
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className={`w-full text-left p-4 flex items-center justify-between font-black ${darkMode ? 'text-white' : 'text-slate-900'} text-sm sm:text-base uppercase tracking-wide transition-all hover:bg-emerald-500/5`}
                      >
                        <span className="flex items-center gap-3">
                          <span className={`w-3 h-3 rounded-full transition-all duration-300 ${isOpen ? 'bg-emerald-600 shadow-md shadow-emerald-600/40 scale-110' : `${darkMode ? 'bg-slate-700' : 'bg-slate-300'}`}`}></span>
                          <span className={isOpen ? 'text-emerald-600' : ''}>{cat.title}</span>
                        </span>
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`p-2 rounded-xl border ${darkMode ? 'bg-slate-800 border-slate-700 text-amber-400 hover:bg-slate-700' : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100'} transition shadow-sm focus:outline-none flex items-center justify-center`}
            title="Toggle Theme"
          >
          ...
          {user ? (
            <div className="relative">
              ...
              >
                <div className="w-7 h-7 bg-gradient-to-tr from-emerald-600 to-teal-500 text-white rounded-full flex items-center justify-center font-black text-xs shadow-sm">
            ...
            <div className="flex items-center gap-1.5">
              <Link 
                to="/admin" 
                className="bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-black px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1.5 transition hover:scale-105 active:scale-95"
              >
              ...
              <Link 
                to="/login" 
                className={`${darkMode ? 'bg-slate-800 hover:bg-slate-700 text-slate-200 border-slate-700' : 'bg-white hover:bg-slate-50 text-slate-800 border-slate-200'} text-[11px] font-black px-3 py-1.5 rounded-xl border flex items-center gap-1.5 transition hover:scale-105 active:scale-95 shadow-sm`}
              >
              ...
              <Link 
                to="/register" 
                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-[11px] font-black px-3 py-1.5 rounded-xl shadow-sm flex items-center gap-1.5 transition hover:scale-105 active:scale-95"
              >
